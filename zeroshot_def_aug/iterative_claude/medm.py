...
def main():
    ...
    output = OUTPUT_DIR / "final_zs_subsample/medm/claude/zs_text_json_100_for_retrieval.json"
    ...
    for item in tqdm(dataset):
            ...
            for _, entities in extracted_sent.items():
                ...
                if entities == []:
                    ...
                    if isinstance(entities[0], str):
                        ...
                        extracted_sent = {"entity": []}
            # make sure the entities are str? 
        ...
        if (count % 100 == 0) or (count == len(dataset)):
            ...
            path = OUTPUT_DIR / "/final_zs_ret/medm/claude/"
            isExist = os.path.exists(path)
            if not isExist:
                print("created path")
                os.makedirs(path)
            with open(
                OUTPUT_DIR / f"/final_zs_ret/medm/claude/zs_ques_ret_{count}.json",
                "w",
            ) as f:
