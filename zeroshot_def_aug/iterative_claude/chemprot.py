...
def main():
    ...
    output = OUTPUT_DIR / "final_zs_subsample/chemprot/claude/zs_text_json_100_for_retrieval.json"
    ...
    for item in tqdm(dataset):
        ...
        if (count % 100 == 0) or (count == len(dataset)):
            ...
            path = OUTPUT_DIR / "/final_zs_ret/chemprot/claude/"
            isExist = os.path.exists(path)
            if not isExist:
                print("created path")
                os.makedirs(path)
            with open(
                OUTPUT_DIR / f"/final_zs_ret/chemprot/claude/zs_ques_ret_{count}.json",
                "w",
            ) as f:
