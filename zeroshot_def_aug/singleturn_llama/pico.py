...
def main():
    ...
    output = OUTPUT_DIR / "final_zs/pico/llama/zs_text_json_100_for_retrieval.json"
    ...
    for item in tqdm(dataset):
        ...
        for sent_id, text in enumerate(sent_text):
            ...
            try:
                ...
                messages += [f"<bot>: {'population'}: [], {'intervention'}: [], {'comparator'}: [], {'outcome'}: []"]
                list_to_retrieve = []
                count_for_no_ents += 1 
    ...
    if (count % 100 == 0) or (count == len(dataset)):
        ...
        path = OUTPUT_DIR / "/final_ret/pico/llama/"
        isExist = os.path.exists(path)
        if not isExist:
            print("created path")
            os.makedirs(path)
        with open(
            OUTPUT_DIR / f"/final_ret/pico/llama/zs_stC_ret_{count}.json",
            "w",
        ) as f:
