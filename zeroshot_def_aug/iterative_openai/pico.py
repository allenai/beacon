...
def main():
    ...
    ddataset = load_dataset("bigbio/ebm_pico", split="test")
    output = OUTPUT_DIR / "final_zs_subsample/pico/gpt4/zs_text_json_100_for_retrieval.json" 
    ...
    for item in tqdm(dataset):
        ...
        for sent_id, text in enumerate(sent_text):
            ...
            for noun_phrase, definition in noun_definitions.items():
                #query_message = [{"role": "user", "content": utils.BOOLEAN_NOUN_PROMPT.format(noun_phrase=noun_phrase, definition=definition, entity_types=ENTITY_TYPES)}]
                query_message = [{"role": "user", "content": utils.BOOLEAN_NOUN_PROMPT_NO_DEF.format(noun_phrase=noun_phrase, entity_types=ENTITY_TYPES)}]
        ...
        if (count % 100 == 0) or (count == len(dataset)):
            ...
            path = OUTPUT_DIR / "/final_zs_ret/pico/gpt4/"
            isExist = os.path.exists(path)
            if not isExist:
                print("created path")
                os.makedirs(path)
            with open(
                OUTPUT_DIR / f"/final_zs_ret/pico/gpt4/zs_ques_{count}.json",
                "w",
            ) as f:
