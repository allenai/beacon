...
def run_fewshot(k, seed):
    ...
    all_responses = {}
    prompt = f"""
        extract the following entity types if present: Condition, Device, Drug, Measurement, Mood, Multiplier, Negation, Observation, Person, Procedure, Qualifier, Reference_point, Scope, Temporal, Value, Visit (alphabetical) from the sentence and return as a Json.
        Each entity is a dictionary of the format {entities_map}. Do not say anything like 'Here are the entities', only output the json.
    """
    ...
    if (count % 100 == 0) or (count == len(dataset)):
        ...
        path = OUTPUT_DIR / "/final_fs_subsample/chia/llama/"
        isExist = os.path.exists(path)
        if not isExist:
            print("creating path")
            os.makedirs(path)
        with open(
            OUTPUT_DIR / f"/final_fs_subsample/chia/llama/fs_text_json_{k}shot_seed{seed}_{count}.json",
            "w",
        ) as f:
