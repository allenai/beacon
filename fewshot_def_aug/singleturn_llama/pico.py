...
def run_fewshot(k, seed):
    ...
    output = OUTPUT_DIR / f"/final_fs_subsample/pico/llama/zs_text_json_5shot_seed{seed}_100.json"
    ...
    for item in tqdm(dataset):
        ...
        for sent_id, this_text in enumerate(sent_text):
            ...
            for i in range(len(formatted_shots)):
                ...
                #     else:
                #         messages += [HUMAN_PROMPT + prompt_ent + content_noun + "\nOutput:" ]
                # else:
                #     if content_noun == "":
                #         messages += [HUMAN_PROMPT + prompt_ent + content + "\nOutput:" ]
                #     else:
                #         messages += [HUMAN_PROMPT + prompt_ent + content + content_noun + "\nOutput:" ]
                #
    ...
    if (count % 100 == 0) or (count == len(dataset)):
        ...
        path = OUTPUT_DIR / "/final_fs_ret/pico/llama/"
        isExist = os.path.exists(path)
        if not isExist:
            print("creating path")
            os.makedirs(path)
        with open(
            OUTPUT_DIR / f"/final_fs_ret/pico/llama/fs_stC_{k}shot_seed{seed}_{count}.json",
            "w",
        ) as f:
