...
def run_fewshot(k, seed):
    ...
    output = OUTPUT_DIR / f"/final_fs_subsample/cdr/claude/zs_text_json_5shot_seed{seed}_100.json"
    ...
    for item in tqdm(dataset):
            ...
            for i in range(len(formatted_shots)):
                ...
                if content == "":
                    if content_noun == "":
                        ...
                        messages += [HUMAN_PROMPT + common.PROMPT_ENT + content_noun + "\nOutput:"]
                else:
                    if content_noun == "":
                        messages += [HUMAN_PROMPT + common.PROMPT_ENT + content + "\nOutput:"]
                    else:   
                        messages += [HUMAN_PROMPT + common.PROMPT_ENT + content + common.PROMPT_NOUN + "\nOutput:"]
        ...
        if (count % 100 == 0) or (count == len(dataset)):
            ...
            path = OUTPUT_DIR / "/final_fs_ret/cdr/claude/"
            isExist = os.path.exists(path)
            if not isExist:
                print("creating path")
                os.makedirs(path)
            with open(
                OUTPUT_DIR / f"/final_fs_ret/cdr/claude/fs_stC_{k}shot_seed{seed}_{count}.json",
                "w",
            ) as f:
