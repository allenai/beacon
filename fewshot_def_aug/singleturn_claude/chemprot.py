...
def run_fewshot(k, seed):
    ...
    output = OUTPUT_DIR / f"/final_fs_subsample/chemprot/claude/zs_text_json_5shot_seed{seed}_100.json"
    ...
    for item in tqdm(dataset):
        ...
        if (count % 100 == 0) or (count == len(dataset)):
            ...
            path = OUTPUT_DIR / "/final_fs_ret/chemprot/claude/"
            isExist = os.path.exists(path)
            if not isExist:
                print("creating path")
                os.makedirs(path)
            with open(
                OUTPUT_DIR / f"/final_fs_ret/chemprot/claude/fs_stC_{k}shot_seed{seed}_{count}.json",
                "w",
            ) as f:
